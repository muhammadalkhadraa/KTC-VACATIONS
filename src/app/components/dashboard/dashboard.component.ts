...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  approvalText(req: HolidayRequest): string {
    console.log(req)
    if (req.manager_status === 'rejected') return `Rejected by manager (ID: ${req.manager_id})`;
    if (req.gm_status === 'rejected') return `Rejected by general manager (ID: ${req.gm_id})`;
    if (req.manager_status === 'pending') return 'Waiting for manager approval';
    if (req.manager_status === 'approved' && req.gm_status === 'pending') return `Approved by manager (ID: ${req.manager_id}), waiting for general manager`;
    if (req.manager_status === 'approved' && req.gm_status === 'approved') return `Approved by manager (ID: ${req.manager_id}) and general manager (ID: ${req.gm_id})`;
    return '';
  }
